					...
					case 'price-break-down': 
						if( !empty($post_option['room-base-price']) ){
							...
							if( $slug == 'price' || empty($hotel_option['booking-price-display']) || $hotel_option['booking-price-display'] == 'start-from' ){
								...
								if(!empty($post_option['room-dom']) && $post_option['room-dom'] == "true"){
                                    $ret .= '<span class="gdlr-tail">' . gdlr_hotel_money_format($post_option['room-base-price']) . ' / ' . __('Night', 'gdlr-hotel') . ' / ' . __('Person', 'gdlr-hotel') . '</span>';
                                }else {
